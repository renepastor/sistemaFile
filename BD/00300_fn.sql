...
--create trigger aud_user before insert on base.personas for each row execute procedure base.aud_user();
--create trigger aud_user before insert on base.parametros for each row execute procedure base.aud_user();




--grant execute on function base.add_usuario(text, text, text, text, text, integer) to root;
...
begin
  ...
  INSERT INTO base.usuarios(pers_id, cuenta, clave, alias) VALUES
  ((select max(pers_id)+1 from base.usuarios), split_part(p_usuario, '@', 1), crypt('#$%&'||p_usuario||'#$%&', gen_salt('bf')), p_alias)
   returning * into usuarios;

  INSERT INTO base.usr_roles(user_id, rol_id, expira, permiso) VALUES
...
create or replace function base.edit_usuario(
  p_pers_id dllave2,
  p_alias text,
  p_cuenta text,
  p_roles_id djson,
...
begin
  ...
  SELECT * into usuarios FROM base.usuarios WHERE cuenta = p_cuenta;
  if (usuarios is null) then
    --registramos nuevo usuario
    INSERT INTO base.usuarios(pers_id, cuenta, clave, alias, usuario) VALUES
  ...
  LOOP
     --buscamos uno a uno el rol del usuario
      v_rol_id := (SELECT id FROM base.usr_roles 
      ...
      ELSE
         ...
         ELSE
           v_estado = 'X';
         END IF;
         UPDATE base.usr_roles SET estado = v_estado 
...
*   **/
--drop function files.auth(p_usuario text, p_clave text);
...
begin
  ...
  if users.clave = crypt(p_clave, users.clave) then
    return ('root', users.pers_id, users.cuenta)::base.jwt;
  else
    return null;
  end if;
end;
...
*   **/
CREATE or replace FUNCTION base.mi_usuario() RETURNS base.usuarios AS $$
  SELECT *
  FROM base.usuarios

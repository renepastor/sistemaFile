      ...
      <template v-slot:activator="{ on, attrs }">
        <v-hover>
            <template v-slot:default="{ hover }">
                <v-btn icon color="primary" :elevation="hover ? 10 : 0" fab dark small v-bind="attrs" v-on="on">
...
<script>
  export default {
    ...
    methods:{
        async editModulo(){
            const modulo = await this.$axios.$post(`${process.env.BASE_URL}/modulos`,this.modulo, {headers: {'Ypfb-App-Access-Token' : process.env.API_KEY} })
            try {
